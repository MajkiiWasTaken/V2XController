...
namespace V2XController
{
    public static class ProtobufParser
    {
        ...
        private static ProtoMessage DetectMessageType(byte[] data)
        {
            ...
            var field30Data = new List<byte[]>();
            var field30InnerFieldNumbers = new HashSet<int>();
            int pos = 0;
                        ...
                        case 2: // Length-delimited
                            if (TryReadVarint(data, ref pos, out ulong length))
                            {
                                if (pos + (int)length <= data.Length)
                                {
                                    ...
                                    // NEW: Analyze field 30
                                    else if (fieldNumber == 30)
                                    {
                                        field30Data.Add(bytes);
                                        int innerPos = 0;
                                        try
                                        {
                                            while (innerPos < bytes.Length)
                                            {
                                                ...
                                                int innerFieldNumber = (int)(innerTag >> 3);
                                                int innerWireType = (int)(innerTag & 0x7);
                                                field30InnerFieldNumbers.Add(innerFieldNumber);
            ...
            if (field30InnerFieldNumbers.Count > 0)
                Console.WriteLine($"[DETECT] Field 30 inner: {string.Join(", ", field30InnerFieldNumbers.OrderBy(x => x))}");

            // **UPDATED DECISION LOGIC**

            // 1. Field 30 disambiguation: Heartbeat vs EmptyResponse
            if (fieldNumbers.Contains(30))
            {
                // Check field 30 content
                bool field30IsEmpty = field30Data.Count > 0 && field30Data[0].Length == 0;
                bool field30HasInnerFields = field30InnerFieldNumbers.Count > 0;

                // EmptyResponse má typicky prázdný obsah nebo jen několik základních polí
                // Heartbeat má obvykle timestamp (field 1) a další data
                bool looksLikeEmptyResponse = field30IsEmpty ||
                    (field30InnerFieldNumbers.Count == 0) ||
                    (field30InnerFieldNumbers.Count <= 2 && !field30InnerFieldNumbers.Contains(1));

                bool looksLikeHeartbeat = field30HasInnerFields &&
                    (field30InnerFieldNumbers.Contains(1) || field30InnerFieldNumbers.Count > 2);

                // If only field 30 + metadata fields (1, 2, 5), decide based on content
                if (!fieldNumbers.Contains(10) && !fieldNumbers.Contains(20))
                {
                    if (looksLikeEmptyResponse)
                    {
                        if (_compiledMessages.TryGetValue("ControllerToRsuMessageData", out var ctrlMsg))
                        {
                            Console.WriteLine("→ ControllerToRsuMessageData (field 30 = empty_response)");
                            return ctrlMsg;
                        }
                    }
                    else if (looksLikeHeartbeat)
                    {
            ...
            foreach (var messageTypeName in messageTypesToTry)
            {
                if (_compiledMessages.TryGetValue(messageTypeName, out var message))
                {
                    try
                    {
                        ...
                        if (testDecode.ContainsKey("empty_response")) score += 15;
        ...
        private static byte[] EncodeMessageToBytes(Dictionary<string, object>? jsonObject, ProtoMessage? message)
        {
            ...
            using (var ms = new MemoryStream())
            {
                ...
                foreach (var field in fields)
                {
                    ...
                    // **NEW: Special handling for empty_response as empty object**
                    if (field?.Name == "empty_response" && field?.Number == 30)
                    {
                        ...
                        {
                            isEmpty = !je.EnumerateObject().Any();
                        }
                        ...
                        if (isEmpty)
                        {
                            Console.WriteLine($"[ENCODE]     empty_response is empty object - encoding as field 30 with length 0");

                            // Write tag for field 30 (length-delimited)
                            ulong tag = ((ulong)30 << 3) | 2UL;
                            WriteVarint(ms, tag);

                            // Write length = 0 (empty message)
                            WriteVarint(ms, 0);
                            continue;
                        }
                    }
                    ...
                    {
                        Console.WriteLine($"[ENCODE]     JsonElement.ValueKind = {jeVal.ValueKind}");

                        // Skip default/empty values EXCEPT for important fields
                        if (IsDefaultOrEmptyValue(jeVal, field?.Type ?? ""))
                        {
                        ...
                        if (field?.IsRepeated == true && jeVal.ValueKind == JsonValueKind.Array)
                        {
                            int arrayLen = jeVal.GetArrayLength();
                            Console.WriteLine($"[ENCODE]     Repeated field, array length: {arrayLen}");

                            if (arrayLen == 0)
                                continue;

                            foreach (var item in jeVal.EnumerateArray())
                            {
                        ...
                        {
                            WriteField(ms, field ?? throw new Exception("Field is null"), jeVal);
                        }
                    }
                    else
                    {
